<#######<Script>#######>
<#######<Header>#######>
# Name: Get-CompoundInterestCalc
# Copyright: Gerry Williams (https://automationadmin.com)
# License: MIT License (https://opensource.org/licenses/mit)
# Script Modified from: n/a
<#######</Header>#######>
<#######<Body>#######>
Function Get-CompoundInterestCalc
{
   <#
.Synopsis
Powershell replacement for https://www.investor.gov/financial-tools-calculators/calculators/compound-interest-calculator
.Description
Powershell replacement for https://www.investor.gov/financial-tools-calculators/calculators/compound-interest-calculator
.Example
Returns: $15,256.36
...
#>
   ...
   {   
      Try
      {
         ...
         if ($Compounds -eq "annually")
         {
            $Payments = 12 * $MonthlyContribution
            $Value = 0 + $InitialInvestment * [math]::pow((1 + $AnnualInterest), $NumberYears) + $Payments * (1 + $AnnualInterest * 0) / $AnnualInterest * ([math]::pow((1 + $AnnualInterest), $NumberYears) - 1)
            $CurrencyValue = "{0:C2}" -f $Value
            Write-Output $CurrencyValue
         }
         else
         {
            $AnnualInterest = $AnnualInterest / 12
            $NumberYears = $NumberYears * 12
            $Payments = $MonthlyContribution
            $Value = 0 + $InitialInvestment * [math]::pow((1 + $AnnualInterest), $NumberYears) + $Payments * (1 + $AnnualInterest * 0) / $AnnualInterest * ([math]::pow((1 + $AnnualInterest), $NumberYears) - 1)
            $CurrencyValue = "{0:C2}" -f $Value
            Write-Output $CurrencyValue
         }
